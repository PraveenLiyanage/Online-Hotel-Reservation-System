        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto py-0 pe-4 border-end border-5 border-primary">
                ...
                <div class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown">VACANCIES</a>
                    <div class="dropdown-menu m-0">
                        <a href="add_vacancies.jsp" class="dropdown-item"> Add Vacancies </a>
                        <a href="search_vacancies.jsp" class="dropdown-item">Manage Vacancies</a>
                       
                    </div>
                </div>
                
                 <a href="home.jsp" class="nav-item nav-link">USER SIDE</a>
            ...
            <div class="row g-0 justify-content-start">
                <div class="col-lg-6">
                    <div class="bg-white text-center p-5">
                        <h1 class="mb-4">vacancies account </h1>
	
	
	
	
	<c:forEach items="${vacantDetail}" var="vac">
	
	<c:set var="ID" value="${vac.vacancies_id}"/>
	<c:set var="type" value="${vac.vacancies_type}"/>
	<c:set var="Price" value="${vac.price}"/>
	<c:set var="availab" value="${vac.available}"/>
	<c:set var="loca" value="${vac.location}"/>	
 
	
	<h4>vacancies ID : </h4> ${vac.vacancies_id}
	<h4>vacancies type : </h4> ${vac.vacancies_type}
	<h4>price : </h4> ${vac.price}
	<h4>Availabale : </h4> ${vac.available}
	<h4>Location : </h4> ${vac.location}
	 
	</c:forEach>
	
	<c:url value="update_Vacancies.jsp" var="vacntupdate">
		<c:param name="id" value="${ID}"/>
		<c:param name="typ" value="${type}"/>
		<c:param name="pric" value="${Price}"/>
		<c:param name="avail" value="${availab}"/>
		<c:param name="locat" value="${loca}"/>
		 
	</c:url>
	<c:url value="delete_Vacancies.jsp" var="vacntdelete">
		<c:param name="id" value="${ID}"/>
		<c:param name="typ" value="${type}"/>
		<c:param name="pric" value="${Price}"/>
		<c:param name="avail" value="${availab}"/>
		...
		<br><br>
	<a href="${vacntupdate}">
	<button class="btn btn-primary w-100 py-3" >Update</button>
	</a><br><br><br>
	<a href="${vacntdelete}">
	<button class="btn btn-primary w-100 py-3" >delete</button>
	</a>
	 </div>
                </div>
            </div>
