            ...
            <div class="row g-0 justify-content-start">
                <div class="col-lg-6">
                    <div class="bg-white text-center p-5">
                        <h1 class="mb-4">feedback account </h1>	
	
	
	<c:forEach items="${feedbackDetails}" var="feed">
	
	<c:set var="ID" value="${feed.feedback_id}"/>
	<c:set var="FEED" value="${feed.feedback}"/>
	<c:set var="MAIL" value="${feed.email}"/>
	<c:set var="NAME" value="${feed.name}"/>
	
 
	
	<h4>feedback ID : </h4> ${feed.feedback_id}
	<h4>feedback : </h4> ${feed.feedback}
	<h4>email : </h4> ${feed.email}
	<h4>name : </h4> ${feed.name}
	
	 
	</c:forEach>
	
	
	<c:url value="update_feedback.jsp" var="feedupdate">
		<c:param name="Id" value="${ID}"/>
		<c:param name="Feed" value="${FEED}"/>
		<c:param name="Email" value="${MAIL}"/>
		<c:param name="Name" value="${NAME}"/>
		 
		 
	</c:url>
	<c:url value="delete_feedback.jsp" var="feeddelete">
		<c:param name="Id" value="${ID}"/>
		<c:param name="Feed" value="${FEED}"/>
		<c:param name="Email" value="${MAIL}"/>
		<c:param name="Name" value="${NAME}"/>
